...
@interface GemCalendarView()<UIScrollViewDelegate,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>
...
@implementation GemCalendarView
{
    ...
    UILabel *_yearMonthLabel;//显示年月
    UIButton *_leftSlideButton;//左滑
    UIButton *_rightSlideButton;//右滑
}
...
-(void)createUI
{
    ...
    //左右滑动按钮
    _leftSlideButton = [[UIButton alloc]initWithFrame:CGRectMake(_yearMonthLabel.frame.origin.x - 30, _yearMonthLabel.center.y - 5, 10, 10)];
    [_leftSlideButton setBackgroundImage:[UIImage imageNamed:@"zuo"] forState:UIControlStateNormal];
    [_leftSlideButton addTarget:self action:@selector(leftSlide:) forControlEvents:UIControlEventTouchUpInside];
    [_calendarView addSubview:_leftSlideButton];
    _rightSlideButton = [[UIButton alloc]initWithFrame:CGRectMake(_yearMonthLabel.frame.origin.x + _yearMonthLabel.frame.size.width + 20, _yearMonthLabel.center.y - 5, 10, 10)];
    [_rightSlideButton setBackgroundImage:[UIImage imageNamed:@"you"] forState:UIControlStateNormal];
    [_rightSlideButton addTarget:self action:@selector(rightSlide:) forControlEvents:UIControlEventTouchUpInside];
    [_calendarView addSubview:_rightSlideButton];
    
    UITapGestureRecognizer *tapGR = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(tapToScroll:)];
    [_calendarView addGestureRecognizer:tapGR];
...
-(void)tapToScroll:(UITapGestureRecognizer *)tap
{
    CGPoint point = [tap locationInView:_calendarView];
    if (CGRectContainsPoint(CGRectMake(_yearMonthLabel.frame.origin.x - 50, _yearMonthLabel.frame.origin.y, 50, _yearMonthLabel.frame.size.height), point)) {
        [self leftSlide:_leftSlideButton];
    }else if (CGRectContainsPoint(CGRectMake(_yearMonthLabel.frame.origin.x + _yearMonthLabel.frame.size.width, _yearMonthLabel.frame.origin.y, 50, _yearMonthLabel.frame.size.height), point)){
        [self rightSlide:_rightSlideButton];
    }else if (CGRectContainsPoint(CGRectMake(_calendarView.frame.size.width - 30, 0, 30, 30), point)){
